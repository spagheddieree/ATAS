...
> **Renamed 2026-09-12.** Previously *Replay Event Verifier*
> (`ReplayEventVerifier.*`, directory `Replay-Event-Verifier/`). The verification
> harness is one job; recording market data faithfully is the lasting one, so the
> name now follows the capability rather than the first experiment.
...
```
src/NFMarketDataRecorder.Core/      netstandard2.0, no package references.
                                   All behaviour. No ATAS dependency.
src/NFMarketDataRecorder.ATAS/      The indicator. The only ATAS types in the repo.
                                   Pure translation, no logic.
src/NFMarketDataRecorder.Harness/   net8.0 CLI: synthetic feed, compare, inspect.
tests/NFMarketDataRecorder.Tests/   73 xunit tests.
...
against `src/NFMarketDataRecorder.ATAS/ApiStub/AtasApiAssumptions.cs`. That file is a
...
dotnet run -c Release --project src/NFMarketDataRecorder.Harness -- \
