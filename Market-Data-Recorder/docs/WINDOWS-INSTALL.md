...
cd ATAS\Market-Data-Recorder
```

## 2 · Confirm the baseline before touching ATAS

```powershell
dotnet build MarketDataRecorder.sln -c Release
dotnet test  MarketDataRecorder.sln -c Release
```
...
Exactly **two** files, from `src\NFMarketDataRecorder.ATAS\bin\Release\net472\`:

| File | Role |
|---|---|
| `NFMarketDataRecorder.ATAS.dll` | the indicator ATAS loads |
| `NFMarketDataRecorder.Core.dll` | all recorder behaviour; the adapter is a thin shell over it |
...
Then restart ATAS. The indicator appears as **NF Market Data Recorder**.
...
|---|---|---|
| Output directory | `%USERPROFILE%\Documents\NFMarketDataRecorder` | a per-run subdirectory is created |
