...
```
ReplayEventVerifier.ATAS      ~230 lines. The only ATAS types in the repository.
                              Pure translation: four callbacks in, four Core calls out.
                              No arithmetic, no state, no decisions.
        │
        ▼
ReplayEventVerifier.Core      netstandard2.0, zero package references.
                              All behaviour. All 73 tests point here.
        ▲
        │
ReplayEventVerifier.Harness   net8.0 CLI. Synthetic feed + compare command.
```
...
| `replay-event-writer` (one) | Dequeue, serialize, write, periodic flush | Touch producer state |
