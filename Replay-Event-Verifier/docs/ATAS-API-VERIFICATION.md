...
This document exists because `src/ReplayEventVerifier.ATAS/ReplayVerifierIndicator.cs`
...
All behaviour lives in `ReplayEventVerifier.Core`, which has no ATAS dependency and is
...
3. Copy `ReplayEventVerifier.ATAS.dll` and `ReplayEventVerifier.Core.dll` into the ATAS
