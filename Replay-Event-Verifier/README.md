# Replay Event Verifier

An ATAS indicator that records **raw market events only** — every individual trade,
every individual market-depth change, and a periodic snapshot of the platform's own
order book — so that ATAS Replay can be checked for event-stream completeness.
...
```
src/ReplayEventVerifier.Core/      netstandard2.0, no package references.
                                   All behaviour. No ATAS dependency.
src/ReplayEventVerifier.ATAS/      The indicator. The only ATAS types in the repo.
                                   Pure translation, no logic.
src/ReplayEventVerifier.Harness/   net8.0 CLI: synthetic feed, compare, inspect.
tests/ReplayEventVerifier.Tests/   73 xunit tests.
...
against `src/ReplayEventVerifier.ATAS/ApiStub/AtasApiAssumptions.cs`. That file is a
...
dotnet run -c Release --project src/ReplayEventVerifier.Harness -- \
