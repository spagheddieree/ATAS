...
5. Add the **NF Market Replay Recorder** indicator to the chart.
...
| Setting | Value |
|---|---|
| Output directory | `%USERPROFILE%\Documents\NFMarketReplayRecorder` |
...
```powershell
dotnet run -c Release --project src\NFMarketReplayRecorder.Harness -- compare `
  --a "$env:USERPROFILE\Documents\NFMarketReplayRecorder\<...>-1x" `
  --b "$env:USERPROFILE\Documents\NFMarketReplayRecorder\<...>-accel" `
...
```powershell
dotnet run -c Release --project src\NFMarketReplayRecorder.Harness -- compare --a <run-1x-dir> --b <run-accel-dir>
```
