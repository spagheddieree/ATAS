...
```powershell
dotnet run -c Release --project tools/NFMarketReplayRecorder.ApiProbe -- --list
```
...
## Why it is not in `NFMarketReplayRecorder.sln`
...
`src/NFMarketReplayRecorder.ATAS/ApiStub/AtasApiAssumptions.cs` — the adapter's written
